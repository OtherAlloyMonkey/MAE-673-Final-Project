...
for ii = 1:length(k)
    ...
    polestemp = eig(Atemp);
    pCl = -.1*ones(4,1) + imag(polestemp)*1i; 
    Ktemp = acker(Atemp,B,pCl);
    Acltemp = Atemp-B*Ktemp;
    ...
    [ytempcasc3, ttempcasc3, statetempcasc3] = lsim(system,uconcasc3,t);

    Cost(ii) = .5*m*(statetempcon(end,2))^2 + .5*J*statetempcon(end,4)^2 + .5*k(ii)*(statetempcon(end,1)-1)^2 + ...
              (.5*k(ii)*(statetempcon(end,1)-1)^2 - .5*k(ii)*(r*statetempcon(end,3) - statetempcon(end,1)- 1)^2 );

    Costc2(ii) = .5*m*(statetempcasc(end,2))^2 + .5*J*statetempcasc(end,4)^2 + .5*k(ii)*(statetempcasc(end,1)-1)^2 + ...
                (.5*k(ii)*(statetempcasc(end,1)-1)^2 - .5*k(ii)*(r*statetempcasc(end,3) - statetempcasc(end,1) - 1)^2 );

    Costc3(ii) = .5*m*(statetempcasc3(end,2))^2 + .5*J*statetempcasc3(end,4)^2 + .5*k(ii)*(statetempcasc3(end,1)-1)^2 + ...
                (.5*k(ii)*(statetempcasc3(end,1)-1)^2 - .5*k(ii)*(r*statetempcasc3(end,3) - statetempcasc3(end,1) - 1)^2 );
...
figure();
semilogy(k,Cost,'o',k,Costc2,'.',k,Costc3,'x')
legend('Concurrent Filter','Cascade Concurrent Filter','Double Cascade','location','northeast')
xlabel('Spring stiffness')
ylabel('Square Root of Residual Energy','interpreter','latex')
